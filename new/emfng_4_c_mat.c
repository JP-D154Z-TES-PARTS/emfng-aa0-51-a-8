...
volatile const s2 s2s_emfng_MFDD = s2g_glround((1.4)/(256./128./256.)); /* m=emfng,lsb=256/128/256,ofs=,unit=% :EM悪化ﾚﾍﾞﾙ失火率 */
...
volatile const u2 u2s_emfng_MFD1 = (u2)(((0.9)/(2./256./256.))+0.5);    /* m=emfng,lsb=2/256/256,ofs=,unit= :EM悪化ﾚﾍﾞﾙ失火検出率 */
  #endif                                                                /* JEMFHOUKI */
#else                                                                   /* 【触媒OTﾚﾍﾞﾙ限定有】 */
volatile const s2 s2s_emfng_MFDD = s2g_glround((25.)/(256./128./256.)); /* m=emfng,lsb=256/128/256,ofs=,unit=% :EM悪化ﾚﾍﾞﾙ失火率 */
volatile const u2 u2s_emfng_MFD1 = (u2)(((0.9)/(2./256./256.))+0.5);    /* m=emfng,lsb=2/256/256,ofs=,unit= :EM悪化ﾚﾍﾞﾙ失火検出率 */
#endif                                                                  /* JEMFOTLV */
...
volatile const u1 u1s_emfng_MFSTOT = (u1)(((85.)/(1.))+0.5);            /* m=emfng,lsb=1,ofs=,unit=回 :気筒別失火回数判定値 */
...
volatile const u1 u1g_emfng_CATN = (u1)(((1.)/(1.))+0.5);               /* m=emfng,lsb=1,ofs=,unit= :バンク数区別の有無(無:1,有:2) */
...
volatile const s2 s2g_emfng_MFOTL = s2g_glround((25.)/(256./128./256.));    /* m=emfng,lsb=256/128/256,ofs=,unit=% :触媒OTﾚﾍﾞﾙ失火判定下限値 */
...
/* L4(bank区別無) ｶﾂ emfng_CATN=2 はemfokでbank区別として参照するため未対応 */
